...
public class Q2_Tower_of_Hanoi {
    /**
     * Problem Description
    ...
    public int[][] towerOfHanoi(int A) {
        int[][] towers = new int[(int)(Math.pow(2, A) - 1)][3];
        moveDisks(towers, A, 1, 3, 2);
        return towers;
    }

    void moveDisks(int[][] towers, int n, int origin, int destination, int buffer) {
        if (n <= 0) return;

        moveDisks(towers, n - 1, origin, buffer, destination);

        towers[index++] = new int[]{n, origin, destination};

        moveDisks(towers, n - 1, buffer, destination, origin);
    }
}
