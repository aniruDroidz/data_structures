...
public class Q1_Path_in_Directed_Graph {
    ...
    public int solve(int A, int[][] B) {
        ...
        for (int[] ints : B) {
            adjucencyList[ints[0]].add(ints[1]);
        }

        boolean[] visited = new boolean[A+1];

        dfs(1, adjucencyList, visited);
    ...
    private void dfs(int i, ArrayList<Integer>[] adjacencyList, boolean[] visited) {
        visited[i] = true;

        for(int j = 0; j < adjacencyList[i].size(); j++) {
            int v = adjacencyList[i].get(j);
